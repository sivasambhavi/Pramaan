...
def page():
    ...
    with col_sidebar:
        st.markdown(
            '<div style="font-size:9px;font-weight:700;color:#475569;text-transform:uppercase;'
            'letter-spacing:.1em;margin-bottom:10px;">ACTIVE CRISES</div>',
            unsafe_allow_html=True,
        )
        for crisis in ACTIVE_CRISES:
            ...
            bg     = "#0d1f35" if is_sel else "#060f1e"
            border = f"1px solid {sc}88" if is_sel else "1px solid #1e293b"
            bl     = f"4px solid {sc}" if is_sel else f"4px solid {sc}44"

            st.markdown(
                f'<div style="background:{bg};border:{border};border-left:{bl};'
                f'border-radius:8px;padding:8px 10px;margin-bottom:6px;cursor:pointer;">',
                unsafe_allow_html=True,
            )
            if st.button(
                crisis["name"],
                ...
                st.rerun()
            st.markdown(
                f'<div style="font-size:9px;color:{sc};font-weight:700;'
                f'text-transform:uppercase;margin-top:-8px;padding:0 4px 6px 4px;">'
                f'{sev} · {crisis["domain"]}</div></div>',
                unsafe_allow_html=True,
            )
    ...
    with col_main:
        ...
        if not data:
            st.markdown(
                '<div style="color:#475569;font-size:13px;margin-top:40px;text-align:center;">'
        ...
        st.markdown(
            ...
            f'<span style="font-size:9px;color:#475569;">Day {len(subevents)} of conflict · '
            f'Started {event.get("date","")}</span>'
            f'</div>'
            f'<div style="font-size:18px;font-weight:700;color:#f1f5f9;">{event.get("name","")}</div>'
            f'<div style="font-size:11px;color:#64748b;margin-top:6px;line-height:1.6;">'
            ...
            for idx, ind in enumerate(key_inds):
                ...
                with cols[idx]:
                    st.markdown(
                        ...
                        f'<div style="font-size:11px;color:{tc};font-weight:600;">{ti} {trend.replace("_"," ")}</div>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
        ...
        with tab_tl:
            if not subevents:
                st.markdown('<div style="color:#475569;font-size:12px;margin-top:20px;">No sub-events recorded yet.</div>',
                            unsafe_allow_html=True)
            else:
                ...
                for se in reversed(subevents):
                    ...
                    st.markdown(
                        ...
                        f'<span style="font-size:11px;font-weight:600;color:#cbd5e1;">{se.get("name","")}</span>'
                        f'</div>'
                        + (f'<div style="font-size:9px;color:#f97316;margin-top:2px;padding-left:130px;">'
        ...
        with tab_exp:
            if not indicators:
                st.markdown('<div style="color:#475569;font-size:12px;margin-top:20px;">No indicators recorded yet.</div>',
                            unsafe_allow_html=True)
            else:
                ...
                with col_metrics:
                    ...
                    for ind in indicators:
                        ...
                        st.markdown(
                            ...
                            f'<span style="font-size:11px;color:#64748b;">{ind.get("name","")}</span>'
        ...
        with tab_dec:
            if not decisions:
                st.markdown('<div style="color:#475569;font-size:12px;margin-top:20px;">No India decisions recorded yet.</div>',
                            unsafe_allow_html=True)
            else:
                ...
                with col_cards:
                    for scn in scenarios:
                        ...
                        st.markdown(
                            ...
                            f'<div style="font-size:12px;font-weight:700;color:{sc};margin-bottom:3px;">'
                            f'{scn.get("name","")}</div>'
